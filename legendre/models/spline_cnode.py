...
def evaluate_spline(t,c,x_eval):
    ...
    if x_eval >= t[-1]:
        ...
        interval_idx = torch.where((t>x_eval)*(x_eval>=torch.cat((torch.zeros(1,device = t.device),t[:-1]))))[0]-1
        try:
    ...
    if x_eval> (t[-1] + 0.1): #x_eval must be less than the last break point
        ...
        y_ = c[...,interval_idx][:,-1] + c[...,interval_idx][:,-2] * x_shifted + c[...,interval_idx][:,-3] * x_shifted**2 + c[...,interval_idx][:,-4] * x_shifted**3
    return y_[:,None]
...
class SplineCNODEClass(pl.LightningModule):
    ...
    def integrate_ode(self,times,Y,mask,coeffs):
        eval_fun = lambda t: evaluate_spline(times,coeffs,t)
        c0 = torch.zeros(Y.shape[0],self.Nc,device = Y.device)
        outputs = odeint(lambda t,cn: self.ode_fun(t,cn,eval_fun),c0,times, method = self.hparams["method"], options = {"step_size":self.hparams["delta_t"]})
        
        # only return the embedding at the last observed observation
        last_observed_indices = (torch.arange(mask.shape[1],device=mask.device)[None,:].repeat(mask.shape[0],1)*mask).max(1)[1]
    ...
    def validation_epoch_end(self, outputs):
        ...
        if not self.pre_compute_ode:
            x_eval = torch.linspace(times[0],times[-1],1000, device = times.device)
            spline_eval = torch.cat([evaluate_spline(times, coeffs,x_eval_) for x_eval_ in x_eval],-1)
        
            # ----- Plotting the filtered trajectories ---- 
            fig = go.Figure()
            fig.add_trace(go.Scatter(x = x_eval.cpu(),y = spline_eval.cpu()[0],mode = 'lines',name='interpolated spline'))
            fig.add_trace(go.Scatter(x = times.cpu(),y = Y_sample[0].cpu(),mode = 'markers',name='observations'))
    ...
    def add_model_specific_args(cls, parent):
        ...
        parser.add_argument('--Nc', type=float, default=32, help = "Dimension of the hidden vector")
