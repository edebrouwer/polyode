...
def pad_irregular_characters(df):
    ...
    for idx in range(df.shape[0]):
        max_len = np.max(np.array([len(df.iloc[idx]["dim_0"]), len(
            df.iloc[idx]["dim_1"]), len(df.iloc[idx]["dim_2"])])).astype(int)
        stacked_vec = np.stack([np.pad(df.iloc[idx]["dim_0"], (0, max_len-len(df.iloc[idx]["dim_0"]))), np.pad(df.iloc[idx]["dim_1"],
                               (0, max_len-len(df.iloc[idx]["dim_1"]))), np.pad(df.iloc[idx]["dim_2"], (0, max_len-len(df.iloc[idx]["dim_2"])))], -1)
        if len(stacked_vec) > max_len_stacked:
            max_len_stacked = len(stacked_vec)
        stacked_list.append(stacked_vec)

    x = np.stack([np.pad(stack, ((0, max_len_stacked-len(stack)), (0, 0)))
                 for stack in stacked_list])
    return x
...
def mixed_loader():
    ...
    padded = nn.utils.rnn.pad_sequence(
        [torch.Tensor(x_.T) for x_ in x], batch_first=True).numpy()
...
class CharacterTraj(Dataset):
    ...
    def __init__(self, train, irregular_rate=1., spline_mode=False, pre_compute_ode=False, multivariate=False, **kwargs):
        ...
        self.mask = np.random.binomial(
            1, irregular_rate, size=(N, t_len)).astype(bool)
    ...
    def __getitem__(self, idx):
        if self.pre_compute_ode:
            return_dict = {"Tobs": self.xobs, "Yobs": self.sequences[idx, :, 0], "mask": self.mask[idx],
                           "label": self.labels[idx], "embeddings": self.embeddings[idx]}
        elif self.spline_mode:
            return_dict = {"Tobs": self.xobs, "Yobs": self.sequences[idx, :, 0],
        ...
        if self.bridge_ode:
            extra_dict = {"ts": self.ts[idx], "ids": self.ids[idx],
                          "ys": self.ys[idx], "mask_ids": self.mask_ids[idx]}
